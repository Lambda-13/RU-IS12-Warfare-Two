...
/datum/dna/proc/ResetUI(var/defer=0)
	for(var/i=1,i<=DNA_UI_LENGTH,i++)
		switch(i)
			if(DNA_UI_SKIN_TONE)
				SetUIValueRange(DNA_UI_SKIN_TONE,rand(1,180),180,1) // Otherwise, it gets fucked
			else
				UI[i]=rand(0,4095)
	if(!defer)
		UpdateUI()
...
/datum/dna/proc/ResetUIFrom(var/mob/living/carbon/human/character)
	...
	SetUIValueRange(DNA_UI_SKIN_TONE, 35-character.s_tone, 180,    1) // Value can be negative.
